...
초기에는 각 오브젝트가 개별적으로 원소 반응을 처리하도록 구현했지만, 반응 로직이 여러 곳에 중복되며 유지보수가 어려워지는 문제가 있었습니다. 
또한 새로운 반응을 추가할 때 기존 반응 로직까지 함께 수정해야 하는 구조적 문제가 존재했습니다.


이를 해결하기 위해 반응 계산은 ElementReactionResolver, 실제 실행은 ElementReactionManager로 분리했습니다.


그 결과 책임 분리를 통해 유지보수성과 확장성을 향상시킬 수 있었고, 새로운 원소 반응 역시 기존 시스템 수정 없이 추가할 수 있도록 개선했습니다.
